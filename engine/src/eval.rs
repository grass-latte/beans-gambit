...
use chess_lib::board::{Board, Piece, PieceKind, Square};

pub fn simple_piece_value(piece: Piece) -> f32 {
    match piece.kind() {
...
#[rustfmt::skip]
pub fn piece_value_at_position(piece: Piece, position: Square) -> f32 {
...
pub fn eval(board: &Board) -> Score {
    ...
    for (position, piece) in board.pieces().iter() {
        let mut s = piece_value_at_position(piece, position);
